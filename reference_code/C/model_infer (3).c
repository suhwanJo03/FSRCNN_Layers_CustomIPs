...
static const char *WEIGHT_DIR = "D:/FSRCNN/reference_code/C/weights";
static const char *INPUT_BMP  = "D:/FSRCNN/reference_code/C/dog/dog_lr_x4.bmp";
static const char *OUTPUT_BMP = "D:/FSRCNN/reference_code/C/dog/dog_INF_C_int8.bmp";
...
/* ─────────── 레이어 출력값 꺼내기(함수 선언) ─────────── */
static void dump_layer_output(const char *filename, const uint8_t *data, int ch, int h, int w)
{
...
int main(void)
{
    ...
    free(yin);
    dump_layer_output("D:/FSRCNN/reference_code/C/dog/layer0_input_quantized.txt", cur, 1, IN_H, IN_W);

    /* [1] first_part_0 */
    layer1_first(cur, nxt, IN_H, IN_W);
    dump_layer_output("D:/FSRCNN/reference_code/C/dog/layer1_first_output.txt", nxt, 16, IN_H, IN_W);
    uint8_t *tmp = cur; cur = nxt; nxt = tmp;

    /* [2] mid_part_0 */
    layer2_reduce(cur, nxt, IN_H, IN_W);
    dump_layer_output("D:/FSRCNN/reference_code/C/dog/layer2_reduce_output.txt", nxt, 12, IN_H, IN_W);
    tmp = cur; cur = nxt; nxt = tmp;

    /* [3] mid_part_3 */
    layer3_mapping_0(cur, nxt, IN_H, IN_W);
    dump_layer_output("D:/FSRCNN/reference_code/C/dog/layer3_mapping_0_output.txt", nxt, 12, IN_H, IN_W);
    tmp = cur; cur = nxt; nxt = tmp;

    /* [4] mid_part_6 */
    layer4_mapping_1(cur, nxt, IN_H, IN_W);
    dump_layer_output("D:/FSRCNN/reference_code/C/dog/layer4_mapping_1_output.txt", nxt, 12, IN_H, IN_W);
    tmp = cur; cur = nxt; nxt = tmp;

    /* [5] mid_part_9 */
    layer5_mapping_2(cur, nxt, IN_H, IN_W);
    dump_layer_output("D:/FSRCNN/reference_code/C/dog/layer5_mapping_2_output.txt", nxt, 12, IN_H, IN_W);
    tmp = cur; cur = nxt; nxt = tmp;

    /* [6] mid_part_12 */
    layer6_mapping_3(cur, nxt, IN_H, IN_W);
    dump_layer_output("D:/FSRCNN/reference_code/C/dog/layer6_mapping_3_output.txt", nxt, 12, IN_H, IN_W);
    tmp = cur; cur = nxt; nxt = tmp;

    /* [7] mid_part_15 */
    layer7_expand(cur, nxt, IN_H, IN_W);
    dump_layer_output("D:/FSRCNN/reference_code/C/dog/layer7_expand_output.txt", nxt, 16, IN_H, IN_W);
    cur = nxt;

    /* [F] deconv */
    uint8_t *out_img = malloc(OUT_H * OUT_W);
    layerF_deconv(cur, out_img);
    dump_layer_output("D:/FSRCNN/reference_code/C/dog/layerF_deconv_output.txt", out_img, 1, OUT_H, OUT_W);
...
static uint8_t* bmp_load_y(const char *path, int h, int w)
{
    ...
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
            ...
            if (i == 0 && j == 0) {
                printf("DEBUG: (i=%d, j=%d) R=%u, G=%u, B=%u\n", i, j, R, G, B);
                printf("DEBUG: Y = %u\n", y_val);
            }
        }
    }
